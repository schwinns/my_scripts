...
class MPD():
    def __init__(self, N):
        ...
        self.ca3 = [atom for atom in N.bonded_atoms if atom.type == '2'][0]
        self.atoms.append(self.ca3)
        
        ca26 = [atom for atom in self.ca3.bonded_atoms if atom.type == '2']
        for ca in ca26:
            ca5 = [atom for atom in ca.bonded_atoms if atom != self.ca3 and ('7' in atom.bonded_atoms.types or '8' in atom.bonded_atoms.types)]
            if len(ca5) > 0:
                self.ca5 = ca5[0]
                ...
                self.ca1 = [atom for atom in ca.bonded_atoms if atom.type == '2' and atom != self.ca3][0]
                self.atoms.append(self.ca1)
                self.ca2 = ca
                self.atoms.append(self.ca2)

        self.ca4 = [atom for atom in self.ca5.bonded_atoms if atom.type == '2' and atom != self.ca6][0]
        self.atoms.append(self.ca4)

        # Save other nitrogen
        if N.type == '7':
            self.N2 = [atom for atom in self.ca5.bonded_atoms if atom.type == '7' or atom.type == '8'][0]
            self.atoms.append(self.N2)
        else:
            self.N1 = [atom for atom in self.ca5.bonded_atoms if atom.type == '7' or atom.type == '8'][0]
            self.atoms.append(self.N1)

        # Save aromatic hydrogens
        self.ha1 = [atom for atom in self.ca1.bonded_atoms if atom.type == '3'][0]
        self.atoms.append(self.ha1)
        self.ha2 = [atom for atom in self.ca2.bonded_atoms if atom.type == '3'][0]
        self.atoms.append(self.ha2)
        self.ha4 = [atom for atom in self.ca4.bonded_atoms if atom.type == '3'][0]
        self.atoms.append(self.ha4)
        self.ha6 = [atom for atom in self.ca6.bonded_atoms if atom.type == '3'][0]
        self.atoms.append(self.ha6)
    
        # Save hydrogens on nitrogens
        hn1 = [atom for atom in self.N1.bonded_atoms if atom.type == '4']
        self.hn1a = hn1[0]
        ...
        hn2 = [atom for atom in self.N2.bonded_atoms if atom.type == '4']
        self.hn2a = hn2[0]
        ...
        n_cl = len([atom for atom in self.mol.atoms if atom.type == '6'])
        if n_cl > 0:
            self.name += '_Cl'
    ...
    def assign_charges(self, charges):
        ...
        for a_type in charges.keys():
            charge = round(charges[a_type]['charge'], 6)
            # charge = charges[a_type]['charge']
...
class TMC():
    def __init__(self, C):
        ...
        # Define the aromatic carbons by diagram above
        self.ca1 = [atom for atom in C.bonded_atoms if atom.type == '2'][0]
        self.atoms.append(self.ca1)
        self.ca2 = [atom for atom in self.ca1.bonded_atoms if atom.type == '2'][0]
        self.atoms.append(self.ca2)
        self.ca3 = [atom for atom in self.ca2.bonded_atoms if atom.type == '2' and atom != self.ca1][0]
        self.atoms.append(self.ca3)
        self.ca6 = [atom for atom in self.ca3.bonded_atoms if atom.type == '2' and atom != self.ca2][0]
        self.atoms.append(self.ca6)
        self.ca5 = [atom for atom in self.ca6.bonded_atoms if atom.type == '2' and atom != self.ca3][0]
        self.atoms.append(self.ca5)
        self.ca4 = [atom for atom in self.ca5.bonded_atoms if atom.type == '2' and atom != self.ca6][0]
        self.atoms.append(self.ca4)

        # Save other carbonyl carbons
        self.C2 = [atom for atom in self.ca3.bonded_atoms if atom.type == '1'][0]
        self.atoms.append(self.C2)
        self.C3 = [atom for atom in self.ca5.bonded_atoms if atom.type == '1'][0]
        ...
        self.O1 = [atom for atom in self.C1.bonded_atoms if atom.type == '9'][0]
        self.atoms.append(self.O1)
        self.O2 = [atom for atom in self.C2.bonded_atoms if atom.type == '9'][0]
        self.atoms.append(self.O2)
        self.O3 = [atom for atom in self.C3.bonded_atoms if atom.type == '9'][0]
        ...
        for i,c in enumerate([self.C1, self.C2, self.C3]):
            oh = [atom for atom in c.bonded_atoms if atom.type == '10']
            cl = [atom for atom in c.bonded_atoms if atom.type == '6']
            if len(oh) > 0:
                c_types.append('term')
                n_oh += 1
                setattr(self, f'oh{i+1}', oh[0])
                self.atoms.append(getattr(self, f'oh{i+1}'))
                ho = [atom for atom in oh[0].bonded_atoms if atom.type == '5']
                setattr(self, f'ho{i+1}', ho[0])
                self.atoms.append(getattr(self, f'ho{i+1}'))
            elif len(cl) > 0:
                c_types.append('unreact')
                n_cl += 1
                setattr(self, f'Cl{i+1}', cl[0])
                self.atoms.append(getattr(self, f'Cl{i+1}'))
            else:
                c_types.append('xlink')
    
        if n_oh == 0 and n_cl == 0:
            self.frag_type = 'xlink'
            self.name = 'TMC_C'
        elif n_oh == 0 and n_cl == 1:
            self.frag_type = 'lin'
            if c_types[0] == 'unreact':
                self.name = 'TMC_L_Cl_1'
            elif c_types[1] == 'unreact':
                self.name = 'TMC_L_Cl_2'
            elif c_types[2] == 'unreact':
                self.name = 'TMC_L_Cl_3'
        elif n_oh == 0 and n_cl == 2:
            self.frag_type = 'term'
            if c_types[0] == 'xlink':
                self.name = 'TMC_T_Cl_1'
            elif c_types[1] == 'xlink':
                self.name = 'TMC_T_Cl_2'
            elif c_types[2] == 'xlink':
                self.name = 'TMC_T_Cl_3'
        elif n_oh == 0 and n_cl == 3:
            self.frag_type = 'mono'
            self.name = 'TMC_Cl'
        elif n_oh == 1:
            self.frag_type = 'lin'
            ...
            raise AttributeError(f'cannot have number of oh type = {n_oh}')
